...
class ReleaseCommand(Command):
    ...
    def __init__(self):
        ...
        default_name = Path(os.getcwd()).name.lower().replace("-", "") # noqa
        ...
        self.parser.add_option(
            ...
            type='str',
            help=(
                "The new version number in the format x.y.z. Can also use "
                "--bug/--minor/--major to bump the current version number"
            )
        )
        ...
        self.parser.add_option(
            ...
            default=False,
            help=(
                "This is the first release. Register this package with PyPi, "
                "don't increment the version number, don't write changes to "
                "the changelog"
            )
        )
    ...
    def run(self, options, args):
        ...
        if not previous_version:
            raise CommandError(
                "Could not determine version. Make sure your root __init__.py "
                "file contains '__version__ = \"1.2.3\"'"
            )
        ...
        warnings = [
            log.split("check: ")[1] for log in output.split("\n") if
            "check: " in log
        ]
        if warnings:
            print(
                "Checks on setup.py failed. Messages were:\n%s" % "\n".join(
                    warnings
                )
            )
            sys.exit(1)
        ...
        if not options.no_release:
            ...
            if not os.path.exists(os.path.expanduser("~/.pypirc")):
                print(
                    "Could not find your ~/.pypirc file. "
                    "See http://seed.readthedocs.org/en/latest/"
                    "#pypi-registration for help."
                )
                sys.exit(1)
            else:
                print(
                    "You have a ~/.pypirc file. Assuming the details in there "
                    "are correct"
                )
        ...
        if options.dry_run:
            if options.initial:
                ...
                print(
                    "Would have written %d changes to changelog" % len(changes)
                )
        else:
        ...
        if options.dry_run:
            print(
                "Would have committed changes to: %s" % ", ".join(commit_files)
            )
        else:
        ...
        if options.no_release:
            ...
            if options.dry_run:
                ...
                print(
                    "(This may take a while, grab a cuppa. "
                    "You've done a great job!)"
                )
                if options.initial or options.register:
                    run_command(
                        "python setup.py register %s upload" % setup_args
                    )
                else:
                    run_command(
                        "python setup.py %s upload" % setup_args
                    )
        ...
        if not options.dry_run:
            if not options.push:
                print(
                    "We have made changes, but not pushed. "
                    "Git users should probably do: "
                )
                print("    git push && git push --tags")
            if options.no_release:
                path = '{} / dist / {}-{}.tar.gz'.format(
                    self.project_dir,
                    self.project_name,
                    next_version
                )
                print(
                    "Package was not released (as requested). "
                    "You can find your package here:\n     {}".format(path)
                )
    ...
    def get_next_version(self, options, previous_version):
        ...
        return self.version_bump(previous_version, version_type)
    ...
    def migrate_version_from_init(self, version_file, vcs):
        print(
            "Moving package version out of __init__.py and into to ./VERSION"
        )
        if hasattr(vcs, 'add'):
    ...
    @staticmethod
    def make_changelog_text(changes, next_version):
        text = ""
    ...
    @staticmethod
    def version_bump(version, version_type="bug"):
        """
        ...
        total_components = max(3, len(parsed_version)) # noqa
        ...
        bits[indexes[version_type]] += 1

        # Set the subsequent digits to 0
        for i in range(indexes[version_type] + 1, 3):
            bits[i] = 0
